...
//  	var setTranslate = function(event){
// 		event.stopPropagation();

// 		var moveX = positionX + (screens[0]-event.pageX)/80 +'px'
// 		  , moveY = positionY + (screens[1] - event.pageY)/80 +'px';
		
// 		//根据鼠标基于banner中心点设置图片偏移
// 		$('.j_moveimg').css({left: moveX, top: moveY});
// 		// $('.mobile').css('transform', 'translate3d(' + moveX + ', '+moveY+', 0px)');
// 		// $('.mobile').css('-moz-transform', 'translate3d(' + moveX + ', '+moveY+', 0px)');
// 		// $('.mobile').css('-webkit-transform', 'translate3d(' + moveX + ', '+moveY+', 0px)');
// 	}

//  	//获取鼠标在banner上移动的位移
// 	$(document).on('mousemove', '#banner', setTranslate);
...
;(function(window, document, $){
	...
	Plugin.prototype = {
		...
		editNumber: function(element, el, callback){
			$(document).on('click', element, function(){
		...
		Alert: function(obj, callback){
			...
			if(typeof obj =='string'){
				...
				html = '<div class="alert-box j-alert">'+
							'<h3 class="alert-title">'+parameter.title+'<span class="close j-close">&#xe63b;</span></h3>'+
							'<div class="content">'+
								'<p class="text">'+obj+'</p>'+
								'<p class="fr"><span class="but ok j-ok">'+parameter.okText+'</span></p>'+
							'</div>'+
						'</div>';
				...
				html = '<div class="alert-box j-alert">'+
							...
							'<div class="content">'+
								...
								'<p class="fr"><span class="but ok j-ok">'+parameter.okText+'</span>';
			...
			var top = ($(window).height()-$('.j-alert').height())/2;
			...
			$(document).on('click', '.j-close, .j-ok, .j-cancel, .mask',function(event){
				...
				if(Browser=='IE8'){
					...
					$('.mask').css({opacity: 0});
					$('.j-alert').css({opacity: 0, zIndex: -1});
					setTimeout(function(){
						$('.mask').css({zIndex: -1});
						$('.j-alert').remove();
						$('body').removeAttr("style");
