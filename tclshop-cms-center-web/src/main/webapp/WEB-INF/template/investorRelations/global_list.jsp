...
<title>
   <c:if test="${result.records[0].type eq 1 }">定期经营报告分析</c:if>
   <c:if test="${result.records[0].type eq 2 }">上市公司报告</c:if>
   <c:if test="${result.records[0].type eq 3 }"> 投资机构报告</c:if>
   <c:if test="${result.records[0].type eq 4 }">上市公司管理制度</c:if>
   <c:if test="${result.records[0].type eq 6 }">公平在身边</c:if>
</title>
