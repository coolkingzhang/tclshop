package com.tclshop.utils.cache.redis;
...
public class RedisSessionUtil {
	...
	private static Logger logger = Logger.getLogger(RedisSessionUtil.class);
	/**
	 * 默认过期时间
	 */
	...
	public static boolean saveSession(String sessionId, Map<String, Object> sessionObj, Integer seconds) {
		...
		try {
			if (seconds != null) {
				result = RedisUtil.save(sessionId, sessionObj, seconds);
			} else {
				result = RedisUtil.save(sessionId, sessionObj, DEFAULT_SESSION_TIMEOUT);
			}
		} catch (Exception e) {
	...
	public static boolean deleteSession(String sessionId) {
		boolean result = false;
		try {
			result = RedisUtil.del(sessionId);
		} catch (Exception e) {
	...
	public static Map<String, Object> getSession(String sessionId) {
		...
		try {
			sessionObj = (Map<String, Object>) RedisUtil.get(sessionId);
		} catch (Exception e) {
	...
	public static boolean updateSessionTime(String sessionId, Integer seconds) {
		...
		try {
			if (seconds == null) {
				result = RedisUtil.expire(sessionId, DEFAULT_SESSION_TIMEOUT);
			} else {
				result = RedisUtil.expire(sessionId, seconds);
			}
		} catch (Exception e) {
